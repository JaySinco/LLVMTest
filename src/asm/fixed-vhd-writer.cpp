...
void release_writer_object(struct writer_object* wo_p) { fclose(wo_p->vhd_fp); }
...
int write_a_vhd_sector(struct writer_object* wo_p, const int64_t lba,
                       const struct vhd_sector* sector_p)
{
...
enum writer_error get_last_error(const struct writer_object* wo_p) { return wo_p->last_error; }
...
#define __err(s, ...) _err_msg(s, ##__VA_ARGS__);
    ...
    const int FLAG_SET_VHD = 1;
    const int FLAG_SET_DATA_FILE = 2;
    const int FLAG_SET_LBA = 4;
    int last_option_flag = 0; /* 3 options must be set up */
    ...
    for (i = 1; i < argc; i++) {
        for (j = 0; argv[i][j] != '\0'; j++) {
            ...
            switch (states) {
                ...
                case 1: /* after accept '-' */
                    if (cc == 'w' && nc == '\0')
                        states = 2; /* VHD file name (write) */
                    else if (cc == 'a' && nc == '\0')
                        states = 3; /* LBA */
                    else if (cc == 'r' && nc == '\0')
                        states = 4; /* data file name (read) */
                    else if (cc == 'h' && nc == '\0')
                        states = 5; /* usage help */
                    else {
                        __err("Invaild option \'%s\'", argv[i]);
                        return -1;
                    }
                    break;
                ...
                case 5:
                    __err(
                        "Fixed VHD Writer\n[-h] usage help\n[-r] specify data file name "
                        "(read)\n[-w] specify VHD file name (write)\n[-a] specify LBA to writing "
                        "data\n");
                    return -1;
            }
        }
