                         ...
                         Shape stride, Shape pad, bool use_act, bool use_bn = USE_BATCH_NORM)
{
    ...
    Symbol out = Convolution("conv_" + name, data, conv_w, conv_b, kernel, num_filter, stride,
                             Shape(1, 1), pad);
    if (use_bn) {
...
{
    Symbol conv1 = convolution_layer(name + "_conv1", data, num_filter, Shape(3, 3), Shape(1, 1),
                                     Shape(1, 1), true);
    Symbol conv2 = convolution_layer(name + "_conv2", conv1, num_filter, Shape(3, 3), Shape(1, 1),
                                     Shape(1, 1), false);
