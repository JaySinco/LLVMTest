...
void infotext(char *title, char *content, double interval)
{
    ...
    if (d->solver_iter) {
        ...
        if (solerr == 0) solerr = mju_max(d->solver[ind].improvement, d->solver[ind].gradient);
    }
...
#define X(TYPE, NAME, NR, NC)                                                                 \
    if (!strcmp(#NAME, settings.field) && !strcmp(#TYPE, "mjtNum")) {                         \
        if (settings.index >= 0 && settings.index < m->NR * NC)                               \
            ...
            strcpy(ui0.sect[SECT_WATCH].item[2].multi.name[0], "invalid index");              \
        return;                                                                               \
    }
...
void makerendering(int oldstate)
{
    ...
    mjuiDef defRendering[] = {
        ...
        {mjITEM_SELECT, "Label", 2, &(vopt.label),
         "None\nBody\nJoint\nGeom\nSite\nCamera\nLight\nTendon\nActuator\nConstraint\nSkin\nSelecti"
         "on\nSel Pnt\nForce"},
        {mjITEM_SELECT, "Frame", 2, &(vopt.frame), "None\nBody\nGeom\nSite\nCamera\nLight\nWorld"},
    ...
    for (i = 0; i < mjMIN(m->ncam, mjMAXUIMULTI - 2); i++) {
        ...
        // check string length
        if (strlen(camname) + strlen(defRendering[1].other) >= mjMAXUITEXT - 1) break;
...
void makejoint(int oldstate)
{
    ...
    for (i = 0; i < m->njnt && itemcnt < mjMAXUIITEM; i++)
        if ((m->jnt_type[i] == mjJNT_HINGE || m->jnt_type[i] == mjJNT_SLIDE)) {
            ...
            if (!vopt.jointgroup[mjMAX(0, mjMIN(mjNGROUP - 1, m->jnt_group[i]))]) continue;

            // set data and name
            defSlider[0].pdata = d->qpos + m->jnt_qposadr[i];
            if (m->names[m->name_jntadr[i]])
                mju_strncpy(defSlider[0].name, m->names + m->name_jntadr[i], mjMAXUINAME);
            else
                sprintf(defSlider[0].name, "joint %d", i);

            // set range
            if (m->jnt_limited[i])
                sprintf(defSlider[0].other, "%.4g %.4g", m->jnt_range[2 * i],
                        m->jnt_range[2 * i + 1]);
...
void makecontrol(int oldstate)
{
    ...
    for (i = 0; i < m->nu && itemcnt < mjMAXUIITEM; i++) {
        ...
        if (!vopt.actuatorgroup[mjMAX(0, mjMIN(mjNGROUP - 1, m->actuator_group[i]))]) continue;

        // set data and name
        defSlider[0].pdata = d->ctrl + i;
        if (m->names[m->name_actuatoradr[i]])
            mju_strncpy(defSlider[0].name, m->names + m->name_actuatoradr[i], mjMAXUINAME);
        else
            sprintf(defSlider[0].name, "control %d", i);

        // set range
        if (m->actuator_ctrllimited[i])
            sprintf(defSlider[0].other, "%.4g %.4g", m->actuator_ctrlrange[2 * i],
...
mjtNum timer(void) { return (mjtNum)(1000 * glfwGetTime()); }
...
void updatesettings(void)
{
    ...
    // physics flags
    for (i = 0; i < mjNDISABLE; i++) settings.disable[i] = ((m->opt.disableflags & (1 << i)) != 0);
    for (i = 0; i < mjNENABLE; i++) settings.enable[i] = ((m->opt.enableflags & (1 << i)) != 0);
...
void uiEvent(mjuiState *state)
{
    ...
    if ((state->dragrect == ui0.rectid) ||
            ...
            switch (it->itemid) {
                ...
                case 9:  // Full screen
                    ...
                    // currently windowed: switch to full screen
                        ...
                        // switch
                        glfwSetWindowMonitor(window, glfwGetPrimaryMonitor(), 0, 0, vmode.width,
        ...
        if (it != NULL || (state->type == mjEVENT_KEY && state->key == 0)) return;
    }
...
void render(GLFWwindow *window)
{
    ...
    if (!m) {
        ...
        if (settings.loadrequest)
            ...
            mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, rect, "Drag-and-drop model file here", 0,
                        &con);
...
int main(int argc, const char **argv)
{
    ...
    std::string arg1 = (__DIRNAME__ / "env/assets/hopper.xml").string();
