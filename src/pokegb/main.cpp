...
#define OP8_NX8_REL(_) OP4_NX8(_, 1) OP4_NX8(_ + 32, 1) tmp8 = reg8_access(0, 0, OPCREL(_));

#define OP64_REL(_)    \
    case _ ... _ + 55: \
        OP8_REL(_ + 56) OPCREL(_);

#define OP9_IMM_PTR(_)      \
    OP8_REL(_) case _ + 70: \
    operand = opcode & 64 ? mem8(PC++) : tmp8;
...
uint8_t opcode, opcrel, tmp8, operand, carry, neg, *rom0, *rom1, io[512], video_ram[8192],
    ...
    *reg8_group[] = {reg8 + 1, reg8, reg8 + 3, reg8 + 2, reg8 + 5, reg8 + 4, &F, &A}, &IF = io[271],
    &LCDC = io[320], &LY = io[324], IME, halt;

uint8_t const* key_state;

uint16_t PC = 256, *reg16 = (uint16_t*)reg8, &HL = reg16[2], SP = 65534, &DIV = (uint16_t&)io[259],
         ppu_dot = 32, *reg16_group1[] = {reg16, reg16 + 1, &HL, &SP},
...
uint8_t mem8(uint16_t addr = HL, uint8_t val = 0, int write = 0)
{
    ...
    switch (addr >> 13) {
        case 1:
            if (write) rom1 = rom0 + ((val ? val & 63 : 1) << 14);

        case 0:
            return rom0[addr];

        case 2:
            if (write && val <= 3) extrambank = extram + (val << 13);
        ...
        case 7:
            if (addr >= 65024) {
                if (write) {
                    if (addr == 65350)
                        for (int y = 160; --y >= 0;) io[y] = mem8(val << 8 | y);
                    io[addr & 511] = val;
                }
                ...
                if (addr == 65280) {
                    if (~io[256] & 16)
                        return ~(16 + key_state[SDL_SCANCODE_DOWN] * 8 +
                                 key_state[SDL_SCANCODE_UP] * 4 + key_state[SDL_SCANCODE_LEFT] * 2 +
                                 key_state[SDL_SCANCODE_RIGHT]);
                    if (~io[256] & 32)
                        return ~(32 + key_state[SDL_SCANCODE_RETURN] * 8 +
                                 key_state[SDL_SCANCODE_TAB] * 4 + key_state[SDL_SCANCODE_Z] * 2 +
...
{
    return (o &= 7) == 6 ? mem8(HL, val, write) : write ? *reg8_group[o] = val : *reg8_group[o];
}
...
int main()
{
    rom1 = (rom0 = (uint8_t*)mmap(0, 1 << 20, PROT_READ, MAP_SHARED, open("rom.gb", O_RDONLY), 0)) +
           32768;
    ...
    SDL_Renderer* renderer =
        SDL_CreateRenderer(SDL_CreateWindow("pokegb", 0, 0, 800, 720, SDL_WINDOW_SHOWN), -1,
                           SDL_RENDERER_PRESENTVSYNC);
    SDL_Texture* texture =
        SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, 160, 144);
                ...
                *reg16_group1[opcrel] = read16();
                case 0:  // NOP
                    ...
                    *reg16_group1[opcrel] += opcode & 8 ? -1 : 1;
                    ...
                    tmp8 = mem8(PC++);
                    if (carry) PC += (int8_t)tmp8, tick();
                    break;

                case 39:  // DAA
                    carry = tmp8 = 0;
                    if (F & 32 || ~F & 64 && A % 16 > 9) tmp8 = 6;
                    if (F & 16 || ~F & 64 && A > 153) tmp8 |= 96, carry = 1;
                ...
                CALL:
                    tmp = read16();
                    if (carry) opcode & 4 ? push(PC) : tick(), PC = tmp;
                    break;
                ...
                ROTATE:
                    switch (opcode) {
                        ...
                        carry = tmp8 & 1;
                        tmp8 = (opcode & 48) == 32 ? (int8_t)tmp8 >> 1
                                                   : tmp8 / 2 + (opcode & 32   ? 0
                                                                 : opcode & 16 ? (F * 8 & 128)
                                                                               : carry * 128);
                    CARRY_ZERO_FLAGS_U:
                ...
                case 250:
                    ...
                    tmp = opcode & 16;
                    reg8_access(
                        mem8(opcode & 8 ? read16() : 65280 + (opcode & 2 ? *reg8 : mem8(PC++)), A,
                             !tmp),
                        tmp, 7);
                    break;
