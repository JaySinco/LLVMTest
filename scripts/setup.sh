...
if [ ! -f "/usr/bin/python3" ]; then sudo apt-get install -y python3 python3-pip; fi
...
if [ ! -f "/usr/bin/clangd" ]; then
    sudo apt-get install -y clangd-15 clang-format-15 clang-tidy-15 lldb-15 libclang-15-dev
